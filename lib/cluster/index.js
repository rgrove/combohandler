...
ComboCluster.prototype._detachEvents = function () {
    ...
    this.process.removeAllListeners('SIGUSR2');
...
ComboCluster.prototype._bindProcess = function () {
    ...
    this.process.on('SIGTERM', boundGracefulShutdown);
    this.process.on('SIGUSR2', this.restartWorkers);
};

ComboCluster.prototype._bindCluster = function () {
    this.cluster.on('fork',       this._workerForked.bind(this));
    this.cluster.on('online',     this._workerOnline.bind(this));
    this.cluster.on('listening',  this._workerListening.bind(this));
    this.cluster.on('disconnect', this._workerDisconnected.bind(this));
    this.cluster.on('exit',       this._workerExited.bind(this));
};
